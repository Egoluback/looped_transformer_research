import torch
import torch.nn as nn
from nano_gpt import GPT2Model, GPT2Config, LayerNorm

MAX_NUM_CLASS = 2  # for openML classification task

def build_model(conf):
    if conf.family == "gpt2":
        model = TransformerModel(
            n_dims=conf.n_dims,
            n_positions=conf.n_positions,
            n_embd=conf.n_embd,
            n_layer=conf.n_layer,
            n_head=conf.n_head,
            pred_type=conf.pred_type,
            use_ssm=conf.use_ssm
        )
    elif conf.family == 'gpt2_loop':
        model = TransformerModelLooped(
            n_dims=conf.n_dims,
            n_positions=conf.n_positions,
            n_embd=conf.n_embd,
            n_layer=conf.n_layer,
            n_head=conf.n_head,
            loop_func=conf.loop_func,
            pred_type=conf.pred_type,
            save_n_tokens=conf.save_n_tokens,
            feed_block=conf.feed_block,
            use_ssm=conf.use_ssm,
            skip_connection=conf.skip_connection
        )
    elif conf.family == 'gpt2_tying':
        model = TransformerModelTying(
            n_dims=conf.n_dims,
            n_positions=conf.n_positions,
            n_embd=conf.n_embd,
            n_layer=conf.n_layer,
            n_head=conf.n_head,
            use_ssm=conf.use_ssm
        )
    else:
        raise NotImplementedError

    return model


class TransformerModel(nn.Module):
    def __init__(self, n_dims, n_positions, n_embd=128, n_layer=12, n_head=4, pred_type='regression', use_ssm=False):

        super(TransformerModel, self).__init__()
        self.freq = 2
        self.ind = 0
        configuration = GPT2Config()
        configuration.block_size = self.freq * n_positions + 1
        configuration.n_layer = n_layer
        configuration.n_head = n_head
        configuration.n_embd = n_embd
        configuration.dropout = 0.0
        configuration.bias = True
        configuration.dropout = 0.
        configuration.use_ssm = use_ssm
        self.configuration = configuration

        self.n_positions = n_positions  # n = points in this setting
        self.n_dims = n_dims  # input dimension, d_in
        self.n_embd = n_embd  # d
        self.n_layer = n_layer
        self._pred_type = pred_type

        self._read_in = nn.Linear(n_dims, n_embd)
        self._backbone = GPT2Model(self.configuration)
        if self._pred_type == 'regression':
            self._read_out = nn.Linear(n_embd, 1)
        elif self._pred_type == 'classification':
            self._read_out = nn.Linear(n_embd, MAX_NUM_CLASS)  # NOTE: hard-code

        self.print_flag = False

    def _combine(self, xs_b, ys_b):
        """
        :param xs_b: shape [B, n, d_in]
        :param ys_b: shape [B, n]
        :return: shape [B, 2n, d_in + 1]
        """
        B, n, d = xs_b.shape
        device = xs_b.device

        ys_b_wide = torch.cat(
            (
                ys_b.view(B, n, 1),
                torch.zeros(B, n, d-1, device=device),
            ),
            axis=2,
        )

        zs = torch.stack((xs_b, ys_b_wide), dim=2)
        zs = zs.view(B, self.freq * n, d)

        return zs

    def forward(self, xs, ys, add_inputs_embeds=False):
        """
        :param xs: [B, n, d]
        :param ys: [B, n]
        :return:
        """

        B, n, d_in = xs.shape
        zs = self._combine(xs, ys)  # [B, n, d_in], [B, n], [B, n] -> [B, 2n, d_in + 1]
        embeds = self._read_in(zs)  # [B, 2n, d_in + 1] -> [B, 2n, d]

        f_output = self._backbone(
            inputs_embeds=embeds, position_ids=None, rm_pos_embd=False, add_inputs_embeds=add_inputs_embeds)  # [B, 2n, d]
        prediction = self._read_out(f_output)  # [B, 2n, d] -> [B, 2n, 1]
        if self._pred_type == 'regression':
            y = prediction[:, self.ind::self.freq, 0]
        elif self._pred_type == 'classification':
            y = prediction[:, self.ind::self.freq]
        else:
            raise NotImplementedError

        return y


class TransformerModelTying(TransformerModel):
    def __init__(self, n_dims, n_positions, n_embd=128, n_layer=12, n_head=4, use_ssm=False):

        super(TransformerModelTying, self).__init__(
            n_dims, n_positions, n_embd, n_layer, n_head)

        self.configuration.n_layer = 1

        self._backbone = GPT2Model(self.configuration)

        self.print_flag = False

    def f(self, output):
        f_output = self._backbone(inputs_embeds=output)  # [B, 2n + 1, d]
        return f_output

    def forward(self, xs, ys, add_inputs_embeds):
        """
        :param xs: [B, n, d]
        :param ys: [B, n]
        :param n_loop_start: int
        :param n_loops: int
        :return:
        """
        zs = self._combine(xs, ys)  # [B, n, d_in], [B, n], [B, n] -> [B, 2n, d_in + 1]
        embeds = self._read_in(zs)  # [B, 2n, d_in + 1] -> [B, 2n, d]
        output = embeds  # also of shape [B, 2n, d]

        for idx in range(self.n_layer):
            output = self.f(output)
        prediction = self._read_out(output)  # [B, 2n, d] -> [B, 2n, 1]
        y = prediction[:, self.ind::self.freq, 0]  # [B, n]

        return y


class TransformerModelLooped(TransformerModel):
    def __init__(
            self, n_dims, n_positions, n_embd=128, n_layer=1, n_head=4, loop_func='z=f(x+z)', pred_type='regression', save_n_tokens=0, feed_block=False, use_ssm=False, skip_connection=False):

        super(TransformerModelLooped, self).__init__(
            n_dims, n_positions, n_embd, n_layer, n_head, pred_type, use_ssm)
        self.feed_block = feed_block
        self.n_layer = n_layer
        self.loop_func = loop_func
        self.save_n_tokens = save_n_tokens
        self.skip_connection = skip_connection

    def f(self, output, embeds, layer_index, skip_connection_output): # если self.skip_connection = False, то skip_connection_output нулевой
        if self.loop_func == 'z=f(x+z)':
            f_output = self._backbone(inputs_embeds=output+embeds+skip_connection_output, layer_index=layer_index)  # [B, 2n + 1, d]
        elif self.loop_func == 'z=f(x*z)':
            f_output = self._backbone(inputs_embeds=output*embeds*skip_connection_output, layer_index=layer_index)  # [B, 2n + 1, d]
        else:
            raise NotImplementedError
        return f_output

    def get_last_n(self, tokens):
        if self.save_n_tokens == 0:
            return tokens
            
        save_n_tokens = self.save_n_tokens
        if self.save_n_tokens > 0 and self.save_n_tokens < 1:
            save_n_tokens = round(self.save_n_tokens * tokens.shape[1])
            
        saved_tokens = tokens[:, -save_n_tokens:, :] # сохранили n токенов с конца или удалили -n токенов с начала
        if self.loop_func in ['z=f(x+z)']:
            masked_tokens = torch.zeros_like(tokens[:, :-save_n_tokens, :])
        elif self.loop_func in ['z=f(x*z)']:
            masked_tokens = torch.ones_like(tokens[:, :-save_n_tokens, :])
        else:
            raise NotImplementedError("Currently we only support loop function z=f(x+z) or z=f(x*z).")

        return torch.cat((masked_tokens, saved_tokens), 1)
    
    def forward(self, xs, ys, n_loop_start, n_loops):
        """
        :param xs: [B, n, d]
        :param ys: [B, n]
        :param n_loop_start: int
        :param n_loops: int
        :return:
        """
        B, n, d_in = xs.shape
        zs = self._combine(xs, ys)  # [B, n, d_in], [B, n], [B, n] -> [B, 2n, d_in + 1]
        embeds = self._read_in(zs)  # [B, 2n, d_in + 1] -> [B, 2n, d]
        if self.loop_func in ['z=f(x+z)']:
            output = torch.zeros_like(embeds)  # also of shape [B, 2n, d]
            skip_connection_output = torch.zeros_like(embeds)
        elif self.loop_func in ['z=f(x*z)']:
            output = torch.ones_like(embeds)  # also of shape [B, 2n, d]
            skip_connection_output = torch.ones_like(embeds)
        else:
            raise NotImplementedError("Currently we only support loop function z=f(x+z) or z=f(x*z).")

        pred_list = []
        layer_index = -1
        if self.feed_block:
            # если слоев несколько и мы после каждого хотим подавать на вход инпут, можно увеличить
            # количество итераций цикла и на каждом шаге говорить трансформеру какой нам нужен слой
            layer_index = 0
            n_loops *= self.n_layer

        for idx in range(n_loops):
            if idx < n_loop_start:  # this will save memory when n_loops large.
                with torch.no_grad():
                    if self.skip_connection: # сохраняем аутпут для skip connection'а
                        output_pr = output
                    output = self.f(self.get_last_n(output), embeds, layer_index, skip_connection_output)
                    if self.skip_connection:
                        skip_connection_output = output_pr
            else:
                if self.skip_connection:
                    output_pr = output
                output = self.f(self.get_last_n(output), embeds, layer_index, skip_connection_output)
                if self.skip_connection:
                    skip_connection_output = output_pr
                
                prediction = self._read_out(output)  # [B, 2n, d] -> [B, 2n, 1]
                if self._pred_type == 'regression':
                    y = prediction[:, self.ind::self.freq, 0]
                elif self._pred_type == 'classification':
                    y = prediction[:, self.ind::self.freq]
                else:
                    raise NotImplementedError
                pred_list.append(y)
            if self.feed_block:
                layer_index = (layer_index + 1) % self.n_layer # переходим к следующему слою

        return pred_list
