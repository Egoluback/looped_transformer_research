...
class BlockSSM(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.ln_1 = LayerNorm(config.n_embd, bias=config.bias)
        self.ssm = SSM(in_features=config.n_embd, dt_rank=256, dim_inner=256, d_state=64)
        self.ln_2 = LayerNorm(config.n_embd, bias=config.bias)
        self.mlp = MLP(config)
...
class GPT2Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.use_ssm:
            self.transformer = nn.ModuleDict(dict(
                ...
                h=nn.ModuleList([BlockSSM(config) for _ in range(config.n_layer)]),
    ...
    def forward(self, inputs_embeds, position_ids=None, rm_pos_embd=False, add_inputs_embeds=False, output_intermediate=False, layer_index=-1):
        ...
        if layer_index != -1:
            block = self.transformer.h[layer_index]
